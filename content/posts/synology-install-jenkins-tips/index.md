...
pubDate: 2025-01-31
author: 'Ethan Yang'
description: '记录在群晖 NAS 中通过 Docker in Docker 安装 Jenkins 遇到的问题，整理容器权限、挂载路径和启动配置的处理经验。'
comment: true
toc: true
tags: ['NAS', 'Tech']
postType: 'article'
...
此时我们需要映射群晖系统上的 docker.sock 文件，这样 jenkins 就可以使用 docker 命令。记住需要 root 用户，不然启动失败。
